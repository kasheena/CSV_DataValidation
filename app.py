...
def main():
    ...
    if uploaded_file2:
        ...
        if df2 is not None:
            ...
            pass_incent_criteria = all(('incent' in str(row['Line Label']).lower() or 'new other cost' in str(row['Line Label']).lower()) and ('G' in str(row.get([col for col in df2.columns if 'PCL' in col][0], row.get('PCL codes', ''))) or 'B' in str(row.get([col for col in df2.columns if 'PCL' in col][0], row.get('PCL codes', '')))) for index, row in df2.iterrows())
            ...
            mismatched_records = df2[~(df2.apply(lambda row: (('sales' in str(row['Line Label']).lower() or 'customer' in str(row['Line Label']).lower()) and any(code in str(row.get([col for col in df2.columns if 'PCL' in col][0], row.get('PCL codes', ''))) for code in sales_criteria_codes)) or 
                                                                ...
                                                                (('incent' in str(row['Line Label']).lower() or 'new other cost' in str(row['Line Label']).lower()) and ('G' in str(row.get([col for col in df2.columns if 'PCL' in col][0], row.get('PCL codes', ''))) or 'B' in str(row.get([col for col in df2.columns if 'PCL' in col][0], row.get('PCL codes', ''))))), axis=1))]
